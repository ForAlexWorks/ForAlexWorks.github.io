...
new IsGrab[MAXPLAYERS+1];
new Srikes[MAXPLAYERS+1];
new Press[MAXPLAYERS+1];
...
public DistanceBetweenPlayersDS(p1)
{
	new Float:firstpl[3];
	new Float:secpl[3]; 
	new farpls;
...
{Press[client]=1; action(client, ATTACKER[client]);}
}
...
CreateConvars()
{
	...
	g_hSur2_default = CreateConVar(
		...
		"1" ,
		"Default selected perk for Survivor, Secondary: 1 = unbreakable, 2 = spirit, 3 = helping hand, 4 = martial artist, 5=decisive strike" ,
		FCVAR_SPONLY|FCVAR_NOTIFY );
...
public Action:Event_TongueGrabPre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_iSur2[victim]==5 && !IsFakeClient(victim)){
		if (Srikes[victim] < 2){
		IsGrab[victim] = 1;
		ATTACKER[victim] = attacker;
		PrintHintText(victim, "Жми [Использовать], чтобы задействовать решающий удар!");
		action(victim, attacker);}}
	if(g_iSur2[victim]==5 && IsFakeClient(victim) && Srikes[victim] < 2){
	ATTACKER[victim] = attacker;
	DistanceBetweenPlayersDS(victim);
	}
...
KillAttacker(client)
{
		...
		new cmdflags = GetCommandFlags("kill");
		SetUserFlagBits(client, ADMFLAG_ROOT);
		SetCommandFlags("kill", cmdflags & ~FCVAR_CHEAT);
		FakeClientCommand(client,"kill");
		SetCommandFlags("kill", cmdflags);
		SetUserFlagBits(client, userflags);	
}

Revive(client, client2)
{
	KillAttacker(client2);
	PrintHintText(client2, "\x03%N\x01 использовал против вас решающий удар!", client);
	CPrintToChatAll("\x05[PerkMod]\x01: \x03%N\x01 использовал \x05Решающий удар\x01 против \x03%N\x01!", client, client2);
	IsGrab[client] = 0;
	Press[client] = 0;
	Srikes[client]++;
	if(Srikes[client] == 2){PrintHintText(client, "Вы использовали решаюший удар максимальное количество раз!");}
}
...
public Handle:Menu_Top (iCid)
{
	...
	else if (g_iSur2[iCid]==5)
		st_perk="Decisive strike";
	else
		st_perk="Not set";
...
public Handle:Menu_ShowChoices (iCid)
{
	...
	{
			Format(st_perk,128,"Decisive Strike, Вырваться из захвата 2 раза за раунд.");
	}
	else
		Format(st_perk,128,"%t", "NotSet");
...
public Handle:Menu_Sur2Perk (client)
{
	...
	else
	{
		...
		Format(st_display,64,"Decisive strike %s",st_current);
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"Позволяет спастись от захвата");
		DrawPanelText(menu, st_display);
		Format(st_display,64,"2 раза за раунд");
		DrawPanelText(menu, st_display);
	}
