...
public OnHunterHighPounce(hunter, survivor, actualDamage, Float:calculatedDamage, Float:height, bool:reportedHigh) {
	if (actualDamage>35){
		...
		CPrintToChatAll("{green}★★★{default} Охотник {red}\x05%N\x01 {default}прыгнул с огромной высоты (%i) на выжившего {green}\x04%N\x01 {default}, чем вывел его из строя!", hunter, actualDamage, survivor);
		return;
	}	
    if(actualDamage > 15)
        CPrintToChatAll("{green}★{default} Охотник {red}\x05%N\x01 {default}прыгнул с большой высоты (%i) на выжившего {green}\x04%N\x01 {default}.", hunter, RoundFloat(height), survivor);
}
